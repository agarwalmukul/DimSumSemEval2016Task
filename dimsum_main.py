...
class BIO:
    ...
    def __init__(self, bio, label=None, pos=None, word=None):
        ...
        if self.label == None: 
            ...
            if self.bio == 'O': 
                self.numeric_label = valid_items[self.label] + 100
            if self.bio == 'B': 
                self.numeric_label = valid_items[self.label] + 200  
            elif self.bio == 'I':
                self.numeric_label = valid_items[self.label] + 300
            elif self.bio == 'o': 
                self.numeric_label = valid_items[self.label] + 400 
            elif self.bio == 'b': 
                self.numeric_label = valid_items[self.label] + 500  
            else: 
                self.numeric_label = valid_items[self.label] + 600  
...
def numeric_label_to_BIO(num):  
    ...
    if num < 100: 
        ...
        ss = valid_items_rev[num - 100]
        return BIO('O', ss)
    elif num < 300: # B
        ss = valid_items_rev[num - 200]
        return BIO('B', ss)
    elif num < 400:  # I
        ss = valid_items_rev[num - 300]
        return BIO('I', ss)
    elif num < 500: # b
        ss = valid_items_rev[num - 400]
        return BIO('b', ss)
    elif num < 600:  # i
        ss = valid_items_rev[num - 500]
        return BIO('i', ss)
    else: # o
        ss = valid_items_rev[num - 600]
        return BIO('o', ss)
...
class MWE(pyvw.SearchTask):
    ...
    def make_example(self, sentence, n):
        ...
        tmp = '<s>'
        caps = "t" if w[0].isupper() else  caps = "f"
        digit = "t" if w.isdigit() else digit = "f"
        ...
        caps_n1 = "t" if ((n+1) <= len(sentence)) and sentence[n+1][0].isupper()) else "f" 
